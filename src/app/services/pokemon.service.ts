...
export class PokemonService {
  ...
  pokemons= [
    ...
    },
    {
      ...
      "description": "There is a bud on this Pokemon's back. To support its weight, Ivysaur's legs and trunk grow thick and strong. If it starts spending more time lying in the sunlight, it's a sign that the bud will bloom into a large flower soon.  There is a bud on this Pok�mon's back. To support its weight, Ivysaur's legs and trunk grow thick and strong. If it starts spending more time lying in the sunlight, it's a sign that the bud will bloom into a large flower soon.",
  ...
  getEvoluciones(index: number){
    //index Representa la posicion del pokemon seleccionado
    var evoluciones = [];
    //Llenar array de objetos (evoluciones) con pokemon de la evolución del seleccionado
    this.pokemons[index].evolutions.forEach(element => {
      evoluciones.push(element);
    })
    return evoluciones;
  }

  getImageReal(nombreEvolucion: string) {
    //Venasaur
    for (let i = 0; i < this.pokemons.length; i++) {
      if(this.pokemons[i].name == nombreEvolucion){
        return this.pokemons[i].art_url;
      }
  }
}

  updateSelectedPokemon(pokemon: Object){
